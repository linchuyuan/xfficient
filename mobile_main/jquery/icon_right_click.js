...
var name;
var name_for_sent;
$(".drag_icon").click(function(event){ 
id = event.target.id; 
name_for_sent = $("#"+id).attr('data-value');
});
			...
			$(".nearby_result").click(function(event){
        			...
        			$.ajax({
                			url: "jquery/php/sent_my_file.php",
               				type: "POST",
                			data: { target_file:name_for_sent , target_nearby:nearby_target },
                			success: function (result) { alert (name_for_sent +" sent to "+nearby_target);}
        			});
			});
...
$(".icon_right_click_icon").click(function (event) {
...
$(".video").click(function (event) {
    ...
    $(".video_box #div_video_popup").attr("src","/mobile_main/session/video_play.php?target="+path);
...
$(".audio").click(function (event) {
    ...
    name = event.target.id;
    var path = $("#"+name).attr("data-rel");
    $(".audio_box #div_audio_popup").attr("src","/mobile_main/session/audio_play.php?target="+path);
...
$(".doc").click(function (event) {
    ...
    name = event.target.id;
    var path = $("#"+name).attr("data-rel");
    $(".doc_box #div_doc_popup").attr("src","/mobile_main/session/doc_play.php?target="+path);
